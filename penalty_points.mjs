...
const relativeData = rawData
	.flatMap((d) => [
		...
		},
		{
			...
			points: -d.points,
			date: d.date_expired
				? new Date(d.date_expired)
				: add(new Date(d.date_added), { years: 1 }),
			color: drivers[d.driver].color,
			dashed: !!drivers[d.driver].dashed,
		},
	])
...
const numDays = 370
const minX = add(new Date(), { days: -numDays })
const maxX = add(new Date(), { days: numDays })
const defaultDriverOptions = {
	x: "date",
	y: "points",
	z: "driver",
	stroke: "color",
	opacity: (d) => (d.date < minX ? 0.2 : 1),
}
